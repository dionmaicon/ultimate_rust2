...
fn main() {
    ...
    let consumer: Vec<_>  = numbers.into_iter().map( | x | x * 3).collect::<_>();
    println!("{:?}", consumer); // should print [3, 6, 9, 12]
    ...
    let transformed: Vec<String> = words.into_iter().filter( |w| !w.contains("h")).map( |w| w.to_uppercase()).collect();
